...
export default function ClientModal({ isOpen, onClose, onSave, client, clients }: ClientModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!validateForm()) {
            // ✨ Mostrar toast para error de validación de campos
        ...
        if (duplicateClient) {
            // setErrors({ phone: `Este teléfono ya está registrado para ${duplicateClient.name}` }); // ❌ Se elimina la asignación a errors para evitar duplicidad visual
            // ✨ Mostrar toast para error de teléfono duplicado
        ...
        if (result.error) {
            // setErrors({ submit: result.error }); // ❌ Se elimina la asignación a errors para evitar duplicidad visual
            // ✨ Mostrar toast para error del servidor/API
            ...
            toast.success(`Cliente ${client ? 'actualizado' : 'creado'} con éxito!`);
        }
    };
    ...
    const handleAddSocialMedia = () => {
        if (!newSocialMediaLink.trim()) {
            ...
            // ✨ Mostrar toast para error de enlace vacío
            toast.warning('El enlace no puede estar vacío.', {
        ...
        if (exists) {
            ...
            // ✨ Mostrar toast para red social duplicada
            toast.warning('Red social duplicada.', {
