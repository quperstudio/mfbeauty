...
export default function ClientProfileModal({ isOpen, onClose, clientId, onEdit }: ClientProfileModalProps) {
  ...
  // Datos para la línea "Creado el..."
  const creationDate = client.created_at ? format(parseDate(client.created_at) || new Date(), 'dd/MM/yyyy', { locale: es }) : 'N/D';
  const createdBy = client.created_by ? (client.created_by.role === 'administrator' ? 'Administrador' : 'Empleado') : 'Desconocido';
  const creationInfo = `Creado el ${creationDate} por ${createdBy}`;
  ...
  const LeftColumnContent = () => (
    <div className="space-y-6 pr-2">
      ...
      <div className="flex items-center justify-between">
        ...
        {/* Clientes Referidos - MOVIMIENTOS: Se mueve aquí y se ajusta el Badge */}
        {client.referrals && client.referrals.length > 0 && (
          <Tooltip>
            <TooltipTrigger asChild>
              <Badge variant="outline" className="text-sm px-3 py-1 font-semibold cursor-default">
                {/* Ícono de Referido (opcional, se mantiene el estilo de contador simple) */}
                Refiere: **{client.referrals.length}**
              </Badge>
            </TooltipTrigger>
            <TooltipContent>
              <p>Clientes referidos por {client.name}</p>
            </TooltipContent>
          </Tooltip>
        )}
      </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          ...
          {client.birthday && (
            <div className="flex items-center text-muted-foreground text-sm">
              ...
              {/* Ajuste: Mostrar solo día (número) y mes (texto) */}
              <span>Cumpleaños: **{format(parseDate(client.birthday) || new Date(), 'dd MMM', { locale: es })}**</span>
            </div>
          )}
          {/* Ajuste: Juntar Creado y Creado por, quitando Badge de rol */}
          <div className="flex items-center text-muted-foreground text-sm">
        ...
        {/* Clientes Referidos - Antiguo emplazamiento (se comenta o elimina) */}
        {/*
        {client.referrals && client.referrals.length > 0 && (
          <div className="mt-4">
            <h3 className="text-md font-medium text-foreground mb-2">
              Clientes Referidos ({client.referrals.length})
            </h3>
            <div className="space-y-2">
        ...
        */}
        {/* Mostrar lista de referidos sólo si hay */}
        {client.referrals && client.referrals.length > 0 && (
          <div className="mt-4">
            <h3 className="text-md font-medium text-foreground mb-2">
