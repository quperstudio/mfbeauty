...
  const { syncTags } = useClientTagsQuery(null); // Hook para sincronización de tags, usado solo para fines de refresco

  // ESTADOS DE PÁGINA
  // -----------------
  const [searchQuery, setSearchQuery] = useState('');
  const [isModalOpen, setIsModalOpen] = useState(false); // Modal de Crear/Editar Cliente
  const [selectedClient, setSelectedClient] = useState<Client | undefined>(); // Cliente para editar
...
  const [selectedClientIds, setSelectedClientIds] = useState<Set<string>>(new Set()); // IDs seleccionados para acciones masivas
  const [isProfileModalOpen, setIsProfileModalOpen] = useState(false); // Modal de Perfil de Cliente
  const [profileClientId, setProfileClientId] = useState<string | null>(null);
  const [isAssignReferrerModalOpen, setIsAssignReferrerModalOpen] = useState(false);
  const [bulkActionLoading, setBulkActionLoading] = useState(false);
  const [clientsWithSelectedTags, setClientsWithSelectedTags] = useState<string[]>([]); // Resultado de filtrar por tags
  const [deleteTarget, setDeleteTarget] = useState<'bulk' | string | null>(null); // ID o 'bulk' para confirmar eliminación
  const [isSmallScreen, setIsSmallScreen] = useState(false);

  // EFECTO: SUBSCRIPCIÓN A CAMBIOS Y TAMAÑO DE PANTALLA
  // --------------------------------------------------
  useEffect(() => {
    // Subscripción a Supabase para invalidar la caché
...
  // EFECTO: FILTRAR POR TAGS SELECCIONADOS
  useEffect(() => {
...
  // CÁLCULOS: FILTRADO Y ORDENAMIENTO
  // --------------------------------
  const filteredAndSortedClients = useMemo(() => {
    let filtered = clients;

    // Aplicar filtro por tipo (visitas, ventas, referidos)
...
        case 'total_spent': aValue = Number(a.total_spent); bValue = Number(b.total_spent); break;
        case 'total_visits': aValue = a.total_visits; bValue = b.total_visits; break;
        case 'last_visit_date': aValue = a.last_visit_date ? new Date(a.last_visit_date).getTime() : 0; bValue = b.last_visit_date ? new Date(b.last_visit_date).getTime() : 0; break;
...
  // Guardado de cliente (delegado a useClientLogic)
...
  // LÓGICA DE ELIMINACIÓN (Individual y Masiva unificada)
  // ----------------------------------------------------
...
        // Si es individual, mostramos el toast de deshacer
        if (!isBulk) {
...
        }
    } catch (error) {
        // El error se maneja en toast.promise
    } finally {
...
    (checked: boolean) => {
      const ids = checked ? new Set(filteredAndSortedClients.map((c) => c.id)) : new Set();
...
        queryClient.invalidateQueries({ queryKey: QUERY_KEYS.clients.all });
        toast.success('Duplicación masiva exitosa', { description: `Se duplicaron ${count} cliente(s) con éxito.` });
      } catch (error) {
        toast.error('Error al duplicar clientes', { description: 'No se pudieron duplicar los clientes. Intenta de nuevo.' });
      } finally {
...
      const csvContent = [
        // Encabezados
        [ 'ID', 'Nombre', 'Teléfono', 'Email', 'Fecha Nacimiento', 'Notas', 'Total Gastado', 'Total Visitas', 'Última Visita', 'WhatsApp', 'Facebook', 'Instagram', 'TikTok', 'ID Referente', 'Fecha Creación' ],
        // Datos
        ...selectedClients.map((c) => [
          c.id, c.name, c.phone, '', c.birthday || '', c.notes || '', c.total_spent.toString(), c.total_visits.toString(), c.last_visit_date || '', c.whatsapp_link || '', c.facebook_link || '', c.instagram_link || '', c.tiktok_link || '', c.referrer_id || '', c.created_at,
        ]),
      ]
...
        queryClient.invalidateQueries({ queryKey: QUERY_KEYS.clients.all });
        toast.success('Referente asignado', { description: `Se actualizó el referente de ${count} cliente(s) con éxito.` });
      } catch (error) {
        toast.error('Error al asignar referente', { description: 'No se pudo asignar el referente. Intenta de nuevo.' });
      } finally {
        setBulkActionLoading(false);
      }
    },
...
  return {
    // Datos y Estados
    clients: filteredAndSortedClients, // Clientes filtrados y ordenados (usados en la tabla)
...
    profileClientId,
    isAssignReferrerModalOpen, setIsAssignReferrerModalOpen,
    bulkActionLoading,
...
    handleEditClient,
    handleSaveClient,
    confirmDeleteClient, // Inicia la eliminación individual
    handleConfirmDelete, // Ejecuta la eliminación (individual o masiva)
    handleDeleteClient: handleConfirmDelete, // Alias para compatibilidad de interfaz (opcional)
    handleSort,
    handleSelectAll,
    handleSelectClient,
    handleViewProfile,
    handleBulkDelete, // Inicia la eliminación masiva
    handleBulkDuplicate,
    handleBulkExport,
